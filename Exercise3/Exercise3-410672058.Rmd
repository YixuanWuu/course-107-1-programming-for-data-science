...
```{r ans11}
as.factor(classData$'公-10-06') -> '公-10-06'
library(stringr)
a1<-str_replace_all(classData$'公-10-06','1','有看公告')
a2<-str_replace_all(classData$'公-10-06','0','沒看公告')
cbind(a1,a2)
```
...
```{r ans21}
library(lubridate)
b1=mdy_hms(hwData$Timestamp)
with_tz(b1,"Asia/Taipei")
```
...
```{r ans22}
deadline <-"2018-10-24 00:00:00"
```

### 2.3
將`deadline`減去`hwData$Timestamp`你會得到一個向量物件，其class為difftime。它的值為兩個Date/Time差了多少秒,但這並不容易理解。使用`difftime()`算出每個作業交件提早了多少**天**, 並存在`hwData$early`裡。

```{r ans23}
as.numeric(deadline)-as.numeric(hwData$Timestamp)
